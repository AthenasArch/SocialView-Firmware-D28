 ...
 */
 //=====================================================================
// Test PDQ ESP32_2432S028 : 2022.08.01 : macsbug
...
//=====================================================================
// ESP32-2432S028 : 2022.08.01 : macsbug
// cfg.freq_write 
// 20000000 : 3618882 : 
// 30000000 : 2715472 : 
// 40000000 : 1892409 : 
...
#include "gfx/lgfx_ESP32_2432S028.h"
...
bool KeyPadLoop(){
    ...
    memset(textBuff, 0, 100);
    
    if (touched) {
        // Limpe apenas uma área específica do display usando um retângulo preenchido com a cor de fundo
        // x, y são as coordenadas do canto superior esquerdo do retângulo, w e h são a largura e altura do retângulo
        int x = 0;
        int w = tft.width();
        int h = 70;
        int y = tft.height() - h;
        tft.fillRect(x, y, w, h, TFT_BLACK); // Preenche a área com a cor de fundo (preto)
        tft.setCursor(x, y);
        tft.setTextColor(TFT_RED); tft.setTextSize(2);
        tft.printf("POSi: \nX = %d\nY = %d", touchX, touchY);
        // sprintf(textBuff, "POSi: \rX = %d\rY = %d", touchX, touchY);
        // tft.drawString(textBuff, x, y);

        // tft.clearDisplay();
        // tft.setTextSize(1);
	    // tft.setCursor(239, 24);
	    // tft.setTextColor(TFT_RED); tft.setTextSize(2);
        // tft.printf("POSi: \nX = %d\nY = %d", touchX, touchY);
        ...
        if(touchX <  80 && touchY < 160){
            ClearKeys(); b_DOWN  = true; vTaskDelay( 25); return true;
        }
        
        // T_WINK_R_PIN -------------------------------------------------------------
        if(touchX > 320){
            ClearKeys(); T_WINK_R_PIN = true; vTaskDelay(200); return true;
        }
    
        // T_WINK_L_PIN ----------------------------------------------------
        if(touchX < 160 ){
            ClearKeys(); T_WINK_L_PIN  = true; vTaskDelay(250); return true;
        }
        
        // T_BLINK_PIN ------------------------------------------------------------
        if( touchX > 160 && touchX < 320 ){
            ClearKeys(); T_BLINK_PIN = true; vTaskDelay(250); return true;
        }
        //--------------------------------------------------------------------
    }
    return false;
